...
| FPL rules page (`/en/help/rules`)                        | HTML   | Deadlines, scoring table, BPS table, chip effects, scoring phases, squad and transfer limits          | Open, scraped                                                                 |
...
Deployment is one Vercel project rooted at the repository root, with `vercel.json` pinning the install and build to `pnpm --filter @fpl/web build` and the output to `apps/web/.next`. The root matters: the site reads the committed lake in `data/` and reads `docs/ARCHITECTURE.md` for `/how-it-works`, both outside `apps/web`, and without the pin Vercel detects `apps/api` and deploys the Fastify server instead of the site.

The refresh endpoints are gated by a shared secret in `REFRESH_TOKEN`, read from `x-refresh-token` or a bearer `authorization` header. With the variable unset the gate is open, which is right locally and wrong in production, so every response carries `unprotected: true` while it is open. There is no scheduler in this repo by design: an external cron service calls these endpoints.
