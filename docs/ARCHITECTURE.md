...
packages/analytics  form, value, fixture difficulty, defence, bonus prediction, squad rules, projection, the metric glossary
...
- **`apps/web`** is Next.js 15 App Router, rendered at build time from the committed lake: `/`, `/players`, `/players/[id]`, `/matches`, `/builder`, `/scout`, `/glossary`, `/how-it-works`, plus `/api/health`, `/api/refresh/fixtures`, and `/api/refresh/rules`. All 590 player profiles are prerendered, each carrying its career table and, where the backfill has run, its international record.
