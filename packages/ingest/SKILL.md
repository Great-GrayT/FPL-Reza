...
- Every identity resolver in spatial/sofascore/identity.ts returns undefined rather than a best guess, and the source counts and logs what it could not join. A wrong player id is worse than a missing row, because nothing downstream can detect it.
